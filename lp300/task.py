...
model.load_weights(os.path.join(data_path, 'p300_weights.h5'));
...
if (not task or task.type == 'master'):
    out = os.path.join(output_path, 'new_p300_model.h5')
    print(out)
    print('save')
    model.save(out)
    model.save_weights(os.path.join(output_path, 'new_p300_weights.h5'))
